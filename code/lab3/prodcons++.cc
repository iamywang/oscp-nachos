...
void Producer(_int which)
{
  ...
  for (num = 0; num < N_MESSG; num++)
  {
    ...
    message->thread_id = which; // message id
    message->value = num;       // meaasge value
...
void ProdCons()
{
  ...
  nempty = new Semaphore("nempty", BUFF_SIZE); // empty
  nfull = new Semaphore("nfull", 0);           // full

  // Put the code to construct a ring buffer object with size
  //BUFF_SIZE here.
  ring = new Ring(BUFF_SIZE); // 缓冲区
  ...
  for (i = 0; i < N_PROD; i++)
  {
    ...
    producers[i] = new Thread(prod_names[i]); // 生产者线程
  ...
  for (i = 0; i < N_CONS; i++)
  {
    ...
    consumers[i] = new Thread(cons_names[i]); // 消费者线程
